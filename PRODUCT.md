...
The two open items from the 2026-08-06 critique are closed, and the shape of the fix is now a standing rule: **no state may be encoded by colour alone.** `Brand.face(for:)` resolves an agent's colour, glyph, and word in one exhaustive switch, so a new state cannot be added without giving it all three and they cannot drift apart; the menu-bar badge adds a silhouette on top (`Brand.worstShape(blocked:silent:)` — triangle → diamond → circle, worst-first). Approve and Deny differ by glyph and by fill weight before they differ by hue. Dwell time escalates in weight as well as colour. Work that adds a status signal is expected to carry all of these, not just the colour.
